...
serve(async req => {
  ...
  try {
    ...
    } else if (clientSystemPrompt && clientSystemPrompt.trim().length > 0) {
      // Respect client-provided system prompt (canonical source: getUniverseSystemPrompt)
      systemPrompt = clientSystemPrompt;
    } else {
      // Canonical 10-point system prompt (mirrors getUniverseSystemPrompt in src/utils/universe/universeUtils.ts)
