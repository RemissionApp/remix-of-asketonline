...
export function extractSections(
  ...
  console.log(
    `Looking for section "${sectionName}" with headers:`,
    possibleHeaders
  );
  ...
  for (const header of possibleHeaders) {
    ...
    const headerPatternWithColon = new RegExp(
      `${header}:\\s*([\\s\\S]*?)(?:\\n\\s*\\n|$)`,
      'i'
    );
    const headerPatternNoColon = new RegExp(
      `${header}\\s*([\\s\\S]*?)(?:\\n\\s*\\n|$)`,
      'i'
    );
    const numberPatternWithDot = new RegExp(
      `\\d\\.\\s*${header}[:\\s]*([\\s\\S]*?)(?:\\n\\s*\\n|\\d\\.\\s*|$)`,
      'i'
    );
    ...
    if (match && match[1]) {
      ...
      console.log(
        `Extracted section text (${sectionText.length} chars): "${sectionText.substring(0, 50)}..."`
      );
      break;
    }
  }
  ...
  if (!sectionText) {
    console.log(
      'Section not found by headers, trying positional extraction...'
    );
    ...
    const paragraphs = horoscopeText
      .split(/\n\s*\n/)
    ...
    if (paragraphs.length >= 5) {
      ...
      if (paragraphs[sectionIndex]) {
        ...
        console.log(
          `Extracted section text by position (${sectionText.length} chars): "${sectionText.substring(0, 50)}..."`
        );
      }
    }
  }
      ...
      if (index !== -1) {
        ...
        for (const nextSectionTitle of Object.values(russianSectionNames)) {
          if (nextSectionTitle !== russianTitle) {
            const nextIndex = horoscopeText.indexOf(
              nextSectionTitle,
              sectionStart
            );
        ...
        console.log(
          `Extracted section text by Russian title (${sectionText.length} chars): "${sectionText.substring(0, 50)}..."`
        );
      }
    }
  }
  ...
  if (!sectionText) {
    console.log(
      'Section not found by headers or position, trying numbered sections...'
    );
    ...
    if (sectionNumber) {
      ...
      if (match && match[1]) {
        sectionText = match[1].trim();
        console.log(
          `Extracted section text by number (${sectionText.length} chars): "${sectionText.substring(0, 50)}..."`
        );
      }
    }
  }
