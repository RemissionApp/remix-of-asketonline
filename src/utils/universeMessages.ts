 ...
 */
export async function generateUniverseAnswer(
  question: string
): Promise<string> {
  ...
  console.log(
    'Generating universe answer for:',
  ...
  try {
    ...
    if (isHoroscopeRequest(question) && userProfile?.birthDate) {
      console.log(
        'Detected horoscope request, processing specialized response'
      );
      try {
        const horoscopeResult = await handleHoroscopeRequest(
          question,
          userProfile,
          language
        );
        ...
        console.warn(
          'Horoscope request detected but no result returned, falling back to regular answer'
        );
