 ...
 */
export async function loadSessionMessages(sessionId: string): Promise<UniverseChatMessage[]> {
  try {
    const { data, error } = await supabase
      .from('universe_chat_messages')
      .select('*')
...
export async function sendMessageToUniverse(
  ...
  try {
    ...
    const { data: universeResponse, error: universeError } = await supabase.functions.invoke('universe-dialogue', {
      body: { 
        question: message,
    ...
    const universeMessageId = await saveMessage(userId, sessionId, universeResponse.answer, 'universe');
