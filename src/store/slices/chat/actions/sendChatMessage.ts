 ...
 */
export const createSendChatMessageAction =
  ...
  async (message: string, messageType: 'user' | 'universe' = 'user') => {
    ...
    try {
      ...
      if (currentVow) {
        ...
        const completedDays = (currentVow.days || []).filter(
      ...
      const { data: dialogueResponse, error: dialogueError } =
      ...
      if (dialogueError) {
        throw new Error(
          `Error from universe-dialogue function: ${dialogueError.message}`
        );
      }
