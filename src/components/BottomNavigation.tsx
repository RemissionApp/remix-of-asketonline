...
export const BottomNavigation: React.FC = memo(() => {
  ...
  const handleNavigation = (screen: 'welcome' | 'language' | 'onboarding' | 'main' | 'create-pact' | 'universe' | 'profile' | 'comparison' | 'meditation' | 'login' | 'signup' | 'universe-call' | 'full-horoscope' | 'numerology', path: string) => {
          ...
          <div className="flex justify-around items-center py-1 max-w-3xl mx-auto">
            ...
            {isPro ? (
              <>
                ...
                >
                  <Phone size={18} />
                  <span className="text-xs">{language === 'ru' ? 'Звонок' : language === 'es' ? 'Llamada' : 'Call'}</span>
                </button>
