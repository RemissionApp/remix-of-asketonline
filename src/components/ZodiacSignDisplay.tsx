...
export const ZodiacSignDisplay: React.FC<ZodiacSignDisplayProps> = ({
  ...
  const getTranslatedElement = () => {
    ...
    if (language === 'ru') {
      switch (element) {
        case 'Fire':
          translatedElement = 'Огонь';
          break;
        case 'Earth':
          translatedElement = 'Земля';
          break;
        case 'Air':
          translatedElement = 'Воздух';
          break;
        case 'Water':
          translatedElement = 'Вода';
          break;
      }
    } else if (language === 'es') {
      switch (element) {
        case 'Fire':
          translatedElement = 'Fuego';
          break;
        case 'Earth':
          translatedElement = 'Tierra';
          break;
        case 'Air':
          translatedElement = 'Aire';
          break;
        case 'Water':
          translatedElement = 'Agua';
          break;
      }
    }
  ...
  return (
    <div className={`flex items-center gap-1 ${className}`}>
      <span
        className={`${sizeClasses.symbol} text-amber-400 ${symbolClassName}`}
      >
        {zodiacInfo.symbol}
      </span>
