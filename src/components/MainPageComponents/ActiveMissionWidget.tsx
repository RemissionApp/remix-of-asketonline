...
export const ActiveMissionWidget: React.FC = () => {
  ...
  // Calculate progress
  const completedCount = mission.progress?.filter(p => p.completed)?.length || 0;
  ...
  const getTitle = () => {
    switch(language) {
      case 'ru': return 'Активная миссия';
      case 'es': return 'Misión activa';
      default: return 'Active mission';
    }
  };
  
  const getReminderText = () => {
    if (todayCompleted) {
      return language === 'ru' ? 'Задача на сегодня выполнена' : 
             language === 'es' ? 'Tarea de hoy completada' : 
             'Today\'s task completed';
    }
    
    return language === 'ru' ? 'Задача на сегодня не выполнена' : 
           language === 'es' ? 'Tarea de hoy pendiente' : 
            ...
            <div className="flex justify-between text-xs text-cosmic-secondary mb-1">
              <span className="flex items-center">
                ...
                {mission.type === 'multi-day' ? 
                  ...
                  language === 'ru' ? 'Одноразовая' : language === 'es' ? 'Única' : 'One-time'
                }
              </span>
              <span>{progress}%</span>
            </div>
