...
export const TopBar: React.FC = memo(() => {
  ...
  return (
    <div className="w-full bg-cosmic-dark/80 backdrop-blur-sm border-b border-cosmic-accent/20 h-16">
      ...
      <div className={`absolute ${isMobile ? 'top-2 right-2' : 'top-4 right-4'} z-10 flex items-center ${isMobile ? 'space-x-1' : 'space-x-2'}`}>
        {/* Sound toggle */}
        <SoundToggle />
        ...
        <div className={`flex items-center ${isMobile ? 'px-2 py-1' : 'px-3 py-1.5'} bg-cosmic-dark/70 backdrop-blur-sm rounded-full border border-cosmic-gold/20 min-w-0`}>
          <CircleDot size={isMobile ? 14 : 16} className="text-cosmic-gold mr-1.5 shrink-0" />
      ...
      <div className={`absolute ${isMobile ? 'top-2 left-2' : 'top-4 left-4'} z-10 flex items-center ${isMobile ? 'space-x-1' : 'space-x-2'} min-w-0`}>
