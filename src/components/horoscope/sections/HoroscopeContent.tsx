...
export const HoroscopeContent: React.FC<HoroscopeContentProps> = ({
  ...
  logger.debug('Component rendered', {
    ...
    horoscopeDescription: horoscope?.description
      ? horoscope.description.substring(0, 50) + '...'
      : 'No description',
  ...
  useEffect(() => {
    ...
    if (horoscope) {
      ...
      if (horoscope.sections) {
        const hasSections = Object.values(horoscope.sections).some(
        ...
        if (!hasSections) {
          logger.error(
            'Horoscope sections are empty or missing',
            horoscope.sections
          );
        }
      } else {
  ...
  const handleSectionComplete = () => {
    if (activeSection < 4) {
      logger.info(
        `Section ${activeSection} complete, advancing to section ${activeSection + 1}`
      );
      setTimeout(() => {
      ...
      <div className="space-y-4">
        <div className="cosmic-gradient-text text-base font-serif leading-relaxed whitespace-pre-wrap p-4 bg-cosmic-dark/40 border border-cosmic-accent/20 backdrop-blur-sm rounded-lg">
          ...
          <TypingEffect
            text={
              ...
              'Извините, в данный момент гороскоп недоступен. Пожалуйста, попробуйте обновить страницу или повторите запрос позже.'
            }
        ...
        {showRawData && rawHoroscopeData && (
          <Accordion
            type="single"
            collapsible
            className="bg-cosmic-dark/40 border border-cosmic-accent/30 rounded-lg overflow-hidden"
          >
  ...
  const sectionConfig = [
    {
      ...
      title:
        language === 'ru' ? 'Общая атмосфера дня' : 'General Day Atmosphere',
    },
    {
      key: 'work_finance',
      title:
    ...
    },
    {
      key: 'daily_advice',
      title:
        language === 'ru' ? 'Практичный совет дня' : 'Practical Daily Advice',
    },
  ];
  ...
  logger.debug('Rendering sections', {
    ...
    sectionsContent: Object.entries(horoscope.sections || {}).map(
      ([key, value]) =>
    ...
    <div className="space-y-4">
      {sectionConfig.map((section, index) => {
        ...
        if (!sectionContent || sectionContent.trim() === '') {
          logger.debug(
            `Section ${section.key} has no content, using fallback text`
          );
        }
        ...
        return (
          activeSection >= index && (
            <HoroscopeSection
              ...
              onComplete={
                activeSection === index ? handleSectionComplete : undefined
              }
            ...
            <AccordionContent className="p-4 bg-black/20">
              {Object.entries(horoscope.sections || {}).map(([key, value]) => (
                <div key={key} className="mb-4">
                  ...
                  <div className="text-xs text-cosmic-secondary whitespace-pre-wrap">
                    {value}
                  </div>
      ...
      {!sectionsLoaded && horoscope.description && (
        <div className="p-4 mt-6 bg-red-900/20 border border-red-500/40 rounded-md">
          <h3 className="text-red-400 font-bold mb-2">
            {language === 'ru'
              ? 'Отладка: Проблема с разделами гороскопа'
              : 'Debug: Issue with horoscope sections'}
          </h3>
