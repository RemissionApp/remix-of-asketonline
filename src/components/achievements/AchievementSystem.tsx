...
interface Achievement {
  ...
  condition: {
    type: 'missions_completed' | 'artifacts_collected' | 'energy_earned' | 'daily_streak' | 'special_event';
    target: number;
    current?: number;
  };
  rewards: {
              ...
              {categoryAchievements.map(achievement => (
                <Card 
                  ...
                  className={cn(
                    ...
                    achievement.unlocked ? 'ring-1 ring-cosmic-gold/50' : 'opacity-60'
                  )}
                >
                  <CardContent className="p-4">
                    <div className="flex items-start gap-4">
                      <div className={cn(
                        ...
                        achievement.unlocked ? 'text-cosmic-gold' : 'text-cosmic-silver/50'
                      )}>
                        {achievement.icon}
                      </div>
                      ...
                      <div className="flex-1 space-y-2">
                        <div className="flex items-center justify-between">
                          <h4 className={cn(
                            'font-semibold',
                            achievement.unlocked ? 'text-white' : 'text-cosmic-silver/70'
                          )}>
                            {achievement.title}
                          </h4>
                          <div className="flex items-center gap-2">
                            ...
                            >
                              {achievement.unlocked ? getText('unlocked') : getText('locked')}
                            </Badge>
                          </div>
                        </div>
                        ...
                        <p className={cn(
                          'text-sm',
                          achievement.unlocked ? 'text-cosmic-silver' : 'text-cosmic-silver/50'
                        )}>
                        ...
                        <div className="space-y-2">
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <span className={cn(
                              achievement.unlocked ? 'text-cosmic-gold' : 'text-cosmic-silver/70'
                            )}>
                        ...
                        <div className="flex items-center gap-2 text-sm">
                          <span className="text-cosmic-silver">{getText('reward')}:</span>
                          <Badge variant="outline" className="bg-cosmic-accent/20 text-cosmic-accent border-cosmic-accent/30">
                            +{achievement.rewards.energyPoints} {getText('energy').toLowerCase()}
                          </Badge>
                          {achievement.rewards.title && (
                            <Badge variant="outline" className="bg-cosmic-gold/20 text-cosmic-gold border-cosmic-gold/30">
                              {achievement.rewards.title}
                            </Badge>
                          )}
                        </div>
