...
export const useMissionCard = (mission: Mission, onComplete?: () => void) => {
  ...
  const [acceptedMission, setAcceptedMission] = useState(
    userProfile?.activeMission?.id === mission?.id
  );
  ...
  useEffect(() => {
    ...
    if (acceptedMission && mission.progress) {
      ...
      setTotalDays(
        mission.type === 'multi-day' ? mission.requirements.length : 1
      );
    }
  ...
  const handleCompleteMission = () => {
    ...
    toast.success(
      language === 'ru'
        ? 'Миссия выполнена! Вы получили награду.'
        : language === 'es'
          ? '¡Misión completada! Has recibido tu recompensa.'
  ...
  const handleAcceptMission = () => {
    ...
    toast.success(
      language === 'ru'
        ? 'Вы приняли новую миссию!'
        : language === 'es'
          ? '¡Has aceptado una nueva misión!'
    ...
    if (mission.requirements) {
      ...
      const initialProgress = Array(requirementsCount)
        .fill(0)
        .map((_, index) => ({
          day: index + 1,
          completed: false,
  ...
  const toggleRequirement = (index: number) => {
    ...
    if (mission.type === 'multi-day') {
      ...
      const previousIncomplete = newStatus
        .slice(0, index)
        .some(status => !status);
    ...
    const requirementsLength = mission.requirements
      ? mission.requirements.length
      : 0;
    const newProgress =
      requirementsLength > 0
        ? Math.floor((completedCount / requirementsLength) * 100)
        : 0;
  ...
  const allCompleted =
    requirementStatus.length > 0 && requirementStatus.every(status => status);
