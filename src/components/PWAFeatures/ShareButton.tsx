...
export const ShareButton: React.FC<ShareButtonProps> = ({
    ...
    try {
      ...
      switch (type) {
        case 'pact':
          if (
            data.pactTitle &&
            data.completedDays !== undefined &&
            data.totalDays !== undefined
          ) {
            shareResult = await share.pactProgress(
              data.pactTitle,
              data.completedDays,
              data.totalDays
            );
  ...
  const getSuccessDescription = () => {
    switch (language) {
      case 'ru':
        return 'Контент был успешно поделён';
      case 'es':
        return 'El contenido se compartió exitosamente';
      default:
        return 'Content was shared successfully';
    }
  };
  ...
  const getCancelTitle = () => {
    switch (language) {
      case 'ru':
        return 'Поделиться отменено';
      case 'es':
        return 'Compartir cancelado';
      default:
  ...
  const getCancelDescription = () => {
    switch (language) {
      case 'ru':
        return 'Вы отменили процесс поделиться';
      case 'es':
        return 'Cancelaste el proceso de compartir';
      default:
        return 'You cancelled the sharing process';
    }
  };
  ...
  const getErrorTitle = () => {
    switch (language) {
      case 'ru':
        return 'Ошибка при попытке поделиться';
      case 'es':
  ...
  const getErrorDescription = () => {
    switch (language) {
      ...
      case 'es':
        return 'No se pudo compartir el contenido. Inténtalo de nuevo.';
      default:
        return 'Failed to share content. Please try again.';
    }
  };
  ...
  return (
    <Button
      size={size}
      className={`w-full bg-cosmic-accent/20 hover:bg-cosmic-accent/30 text-white border border-cosmic-accent/30 font-sans ${className}`}
      onClick={handleShare}
    >
