        ...
        <Table className="border border-cosmic-accent/20">
          <TableHeader>
            <TableRow className="bg-cosmic-dark/80">
              ...
              <TableHead className="text-white text-center">ASKET (Бесплатно)</TableHead>
              <TableHead className="text-white text-center">ASKET PRO (Платная подписка)</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {features.map((feature, index) => (
              <TableRow key={index} className={index % 2 === 0 ? 'bg-cosmic-dark/50' : 'bg-cosmic-dark/30'}>
                <TableCell className="font-medium text-white">{feature.name}</TableCell>
                <TableCell className="text-center text-cosmic-secondary">{feature.free}</TableCell>
                <TableCell className="text-center text-cosmic-gold">{feature.pro}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
        ...
        <div className="grid gap-6">
          ...
          <Card className="border-2 bg-cosmic-dark/50">
            ...
            <CardContent className="pt-6 space-y-4">
              {features.map((feature, index) => (
                <div key={index} className="flex items-start gap-2 py-2 border-b border-cosmic-accent/10">
                  <div>
                    <p className="text-white text-sm font-medium">{feature.name}</p>
                    <p className="text-sm text-cosmic-secondary">{feature.free}</p>
                  </div>
                </div>
              ))}
            </CardContent>
          ...
          <Card className="border-2 border-cosmic-gold bg-cosmic-dark/50 relative">
            ...
            <CardHeader className="text-center bg-gradient-to-b from-cosmic-gold/20 to-cosmic-dark pb-4">
              ...
              <p className="text-lg font-bold mt-2 text-white">$4.99/мес или $29.99/год</p>
            </CardHeader>
            <CardContent className="pt-6 space-y-4">
              {features.map((feature, index) => (
                <div key={index} className="flex items-start gap-2 py-2 border-b border-cosmic-gold/10">
                  <div>
                    <p className="text-white text-sm font-medium">{feature.name}</p>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Card className="bg-cosmic-dark/30 border-cosmic-accent/20">
              <CardContent className="p-4">
                <p className="text-cosmic-secondary italic mb-3">"Медитации полностью изменили мою ежедневную практику. Стоят каждой копейки только за внутреннее спокойствие."</p>
                <p className="text-white font-medium">- Анна К.</p>
              </CardContent>
            </Card>
            <Card className="bg-cosmic-dark/30 border-cosmic-accent/20">
              <CardContent className="p-4">
                <p className="text-cosmic-secondary italic mb-3">"Возможность поддерживать несколько аскез одновременно значительно ускорила мой духовный рост."</p>
